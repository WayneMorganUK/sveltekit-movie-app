import { c as create_ssr_component, e as each, d as add_attribute, f as escape, v as validate_component, b as subscribe, h as set_store_value } from "../../../chunks/index-bac2f3ba.js";
import { a as media_type } from "../../../chunks/store-56c5f052.js";
...
const KnownFor = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<section id="${"known-for"}" class="${"grid mx-auto"}">${films ? `<h3 class="${"flex xl:inline-block justify-center xl:justify-start xl:my-5 text-2xl font-bold"}">Movies
		...
		<div class="${"text-skin-base flex flex-wrap justify-center sm:justify-start sm:flex-nowrap sm:overflow-y-hidden relative"}">${each(films, (movie) => {
    return `<div class="${"w-28 h-56 flex-shrink-0 rounded mb-2 relative bg-secondary m-0.5 sm:mr-3 hover:bg-selected"}"><a class="${"rounded w-28"}"${add_attribute("href", `/movie/${movie.id}`, 0)}><img class="${"object-cover w-28 h-44 rounded-t"}"${add_attribute("src", movie.poster_path ? IMAGE_API$1 + movie.poster_path : "/default.jpg", 0)}${add_attribute("alt", movie.title, 0)}>
	...
	${tv ? `<h3 class="${"flex xl:inline-block justify-center xl:justify-start xl:my-5 text-2xl font-bold"}">TV
		...
		<div class="${"flex flex-wrap justify-center sm:justify-start sm:flex-nowrap sm:overflow-y-hidden relative"}">${each(tv, (show) => {
    return `<div class="${"w-28 h-56 flex-shrink-0 rounded mb-2 relative bg-secondary m-0.5 sm:mr-3 hover:bg-selected"}"><a class="${"rounded w-28"}"${add_attribute("href", `/tv/${show.id}`, 0)}><img class="${"object-cover w-28 h-44 rounded-t"}"${add_attribute("src", show.poster_path ? IMAGE_API$1 + show.poster_path : "/default.jpg", 0)}${add_attribute("alt", show.name, 0)}>
  ...
  return `<section id="${"person"}" class="${"xl:rounded-2xl mx-auto max-w-7xl xl:pb-20 xl:mt-5"}"><div class="${"grid xl:grid-cols-5 xl:grid-rows-auto xl:auto-rows-fr"}"><div class="${"xl:rounded-2xl bg-skin-secondary w-full xl:p-2 xl:col-start-1 xl:col-end-2 xl:row-start-1 xl:row-end-3 mx-auto"}">${person.profile_path ? `<img class="${"mt-4 xl:mt-0 xl:pt-0 w-64 h-96 rounded-2xl mx-auto"}"${add_attribute("src", IMAGE_API + person.profile_path, 0)} alt="${"profile"}">` : `<img class="${"mt-4 xl:pt-0 flex justify-end w-64 h-96 rounded-2xl mx-auto"}" src="${"/person.svg"}" alt="${"profile"}">`}
			...
			<div class="${"text-skin-base pl-8 w-full xl:p-3"}"><h4 class="${"xl:text-2xl mt-2"}">Personal Info</h4>
				...
				${person.also_known_as && person.also_known_as.length ? `<h4 class="${"mt-4 mb-1"}">Also Known As</h4>
					${each(person.also_known_as, (alias) => {
    return `<p class="${"text-skin-muted xl:text-base"}">${escape(alias)}</p>`;
  })}` : ``}</div></div>
...
async function load({ fetch, params }) {
  const res = await (await fetch("../api/postMovie", {
    headers: { "Content-Type": "application/json" },
    method: "POST",
    body: JSON.stringify({ media: "person", id: params.id })
  })).json();
  const person = await res.res;
  const resp = await (await fetch("../api/postKnownFor", {
    headers: { "Content-Type": "application/json" },
    method: "POST",
