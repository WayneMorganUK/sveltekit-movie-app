...
const PersonCard = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  if ($$props.datum === void 0 && $$bindings.datum && datum !== void 0)
    $$bindings.datum(datum);
  return `<section id="${"person-card"}" class="${"w-44 xl:h-108 xl:w-60 my-0.5 xl:my-2 bg-skin-bg xl:rounded-lg"}"><a${add_attribute("href", `/person/${datum.id}`, 0)} class="${"top-0 right-0 text-skin-base xl:rounded-lg"}"><img class="${"h-64 oject-cover w-44 xl:w-60 xl:h-90 xl:rounded-t-lg text-skin-base"}"${add_attribute("src", datum.profile_path === null ? "/person.svg" : IMAGE_API + datum.profile_path, 0)}${add_attribute("alt", datum.name, 0)}>
